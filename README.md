...
你只需要在 Emby 客户端里填写一个 Worker 地址，后端多个 Emby 上游会自动健康检查、按健康度与延迟选优，并在 API 请求失败时自动切换节点。也支持 Emby 场景下的前后端分离：Web 前端资源走前端上游，API / 媒体流走后端上游。

## 功能列表

- **Emby 专用路由**：自动区分媒体流、API、Web 前端资源请求
...
- **网页管理面板**：首次安装向导、节点管理、上游类型、健康检查、状态展示
...
- 首次初始化管理员密码
- 添加 / 编辑 / 删除 Emby 上游
- 在「添加上游」里选择上游类型：`Emby 后端 / API / 媒体流` 或 `Emby Web 前端 / 静态资源`
- 手动触发健康检查
- 查看延迟、健康状态、最后检查时间
